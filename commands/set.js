...
> Returns: \`User points and level information\``)}
    if(!args[1]) {return message.channel.send(`> __**Set Command**__
> Usage:    \`;set @User <points, number> <level/PPC, number>\`
> Returns: \`User points and level information\``)}
    if(!args[2]) {return message.channel.send(`> __**Set Command**__
> Usage:    \`;set @User <points, number> <level/PPC, number>\`
> Returns: \`User points and level information\``)}
   ...
   if(!target){
    ...
    var list = await fetch("https://antidoot.glitch.me/json/"+target.id+"/points/"+args[1]+"/multi/"+args[2]).then(response => response.json());
